...
/*************************************************
* Name:        PQCLEAN_KYBER768_CLEAN_indcpa_keypair_derand
*
...
**************************************************/
void PQCLEAN_KYBER768_CLEAN_indcpa_keypair_derand(uint8_t pk[KYBER_INDCPA_PUBLICKEYBYTES],
    ...
    athos_ip_driver(&skpv.vec[0], &skpv.vec[0], 256, 2);
    athos_ip_driver(&skpv.vec[1], &skpv.vec[1], 256, 2);
    athos_ip_driver(&skpv.vec[2], &skpv.vec[2], 256, 2);
    // PQCLEAN_KYBER768_CLEAN_polyvec_ntt(&e);
    athos_ip_driver(&e.vec[0], &e.vec[0], 256, 2);
    athos_ip_driver(&e.vec[1], &e.vec[1], 256, 2);
    athos_ip_driver(&e.vec[2], &e.vec[2], 256, 2);
...
void PQCLEAN_KYBER768_CLEAN_indcpa_enc(uint8_t c[KYBER_INDCPA_BYTES],
    ...
    PQCLEAN_KYBER768_CLEAN_gen_matrix(at, seed, 1);
    ...
    athos_ip_driver(&sp.vec[0], &sp.vec[0], 256, 2);
    athos_ip_driver(&sp.vec[1], &sp.vec[1], 256, 2);
    athos_ip_driver(&sp.vec[2], &sp.vec[2], 256, 2);
    ...
    athos_ip_driver(&b.vec[0], &b.vec[0], 256, 3);
    athos_ip_driver(&b.vec[1], &b.vec[1], 256, 3);
    athos_ip_driver(&b.vec[2], &b.vec[2], 256, 3);
    //PQCLEAN_KYBER768_CLEAN_poly_invntt_tomont(&v);
    athos_ip_driver(&v, &v, 256, 3);
...
void PQCLEAN_KYBER768_CLEAN_indcpa_dec(uint8_t m[KYBER_INDCPA_MSGBYTES],
    ...
    athos_ip_driver(&b.vec[0], &b.vec[0], 256, 2);
    athos_ip_driver(&b.vec[1], &b.vec[1], 256, 2);
    athos_ip_driver(&b.vec[2], &b.vec[2], 256, 2);

    PQCLEAN_KYBER768_CLEAN_polyvec_basemul_acc_montgomery(&mp, &skpv, &b);

   // PQCLEAN_KYBER768_CLEAN_poly_invntt_tomont(&mp);
   athos_ip_driver(&mp, &mp, 256, 3);
